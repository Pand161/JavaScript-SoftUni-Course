...
describe('E2E tests', function () {
  ...
  describe('Authentication [ 20 Points ]', () => {
    ...
    it('logout makes correct API call [ 5 Points ]', async () => {
      const data = mockData.users[0];
      const { post: loginPost } = await createHandler(endpoints.login, {
        post: data,
      });
  ...
  describe('Catalog [ 20 Points ]', () => {
    it('show empty catalog [ 5 Points ]', async () => {
      ...
      const titles = await page.$$eval('#dashboard-page ul li', (t) =>
    ...
    it('show details [ 5 Points ]', async () => {
      ...
      await page.click(
        `#dashboard-page > ul li:has-text("${data.title}") >> text=Details`
      );
      await page.waitForTimeout(interval);
      ...
      expect(
        await page.isVisible(
          `.book-description > p:has-text("${data.description}")`
        )
      ).to.be.true;
    });
  ...
  describe('CRUD [ 45 Points ]', () => {
    ...
    it('edit does NOT work with empty fields [ 5 Points ]', async () => {
      const data = mockData.catalog[0];
      const { put } = await createHandler(endpoints.details(data._id), {
        get: data,
        put: '',
      });
    ...
    it('delete makes correct API call for logged in user [ 5 Points ]', async () => {
      const data = mockData.catalog[0];
      const { delete: del } = await createHandler(endpoints.details(data._id), {
        get: data,
        delete: '',
      });
  ...
  describe('My Books Page [ 10 Points ]', async () => {
    ...
    it('the logged-in user should be able to see his/her own books (test with 2 books) [ 2.5 Points ]', async () => {
      ...
      const titles = await page.$$eval('#my-books-page ul li', (t) =>
        t.map((s) => s.textContent)
      );
    ...
    it('the logged-in user should be able to view his/her own books - detailed information [ 2.5 Points ]', async () => {
      ...
      const firstImg = await page.getAttribute(
        `.my-books-list > li:has-text("${mock[0].title}") img`,
        'src'
      );
      const secodnImg = await page.getAttribute(
        `.my-books-list > li:has-text("${mock[1].title}") img`,
        'src'
      );
      const thirdImg = await page.getAttribute(
        `.my-books-list > li:has-text("${mock[2].title}") img`,
        'src'
      );
      await page.waitForTimeout(interval);
    ...
    it('the logged-in user should be redirected to the Details page after click on the button "Details" [ 2.5 Points ]', async () => {
      ...
      await page.click(
        `.my-books-list > li:has-text("${data.title}") >> text=Details`
      );
      await page.waitForTimeout(interval);
  ...
  describe('BONUS : Like functionality  [ 15 Points ]', async () => {
    it('Like button is not visible for guest users [ 2.5 Points ]', async () => {
      ...
      await page.waitForTimeout(interval);
      const likes = await page.$$eval('.actions .likes', (t) =>
        t.map((s) => s.textContent)
      );

      expect(await page.isVisible('.actions .button >> text="Like"')).to.be
        .false;
    ...
    it('Like button should be hidden(not visible) after a click on it [ 2.5 Points ]', async () => {
      // Login user
      const { post } = await createHandler(endpoints.like, {
    ...
    it('Like button should increase total likes by 1 after a click on it [ 5 Points ]', async () => {
      ...
      let likes = await page.$$eval('.actions .likes', (t) =>
        t.map((s) => s.textContent)
      );
      expect(likes[0]).to.contains('Likes: 7');
      ...
      await page.waitForTimeout(interval);
      likes = await page.$$eval('.actions .likes', (t) =>
        t.map((s) => s.textContent)
      );
...
async function setupContext(context) {
  ...
  await createHandler(
    endpoints.details('1001'),
    { get: mockData.catalog[0] },
    context
  );
  await createHandler(
    endpoints.details('1002'),
    { get: mockData.catalog[1] },
    context
  );
  await createHandler(
    endpoints.details('1003'),
    { get: mockData.catalog[2] },
    context
  );
  await createHandler(
    endpoints.details('1004'),
    { get: mockData.catalog[3] },
    context
  );
  await createHandler(
    endpoints.details('1005'),
    { get: mockData.catalog[4] },
    context
  );
...
async function createHandler(match, handlers = {}, context) {
  ...
  function urlPredicate(current) {
    const url = (
      current instanceof URL ? current.href : current.url()
    ).toLowerCase();
